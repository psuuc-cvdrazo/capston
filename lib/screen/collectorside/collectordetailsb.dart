...
class _CollectorPointDetailsState extends State<CollectorPointDetails> {
  ...
  Future<void> _sendHistoryRecord() async {
    ...
    try {
      ...
      if (collector != null) {
        ...
        if (response != null) {
          collectorName = response['name'] as String?;
        } else {
          print('Collector not found in the database.');
        }
      }
    } catch (e) {
    ...
    if (collectorName == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Collector name not found.")),
      );
      return;
    }

    // Prepare the payload
    Uri historyUrl = Uri.parse("$supabaseUrl/rest/v1/history");
    ...
    Map<String, dynamic> payload = {
      ...
      "confirmation_time": DateTime.now().toUtc().toIso8601String(),
    };

    // Send the payload to the history table
    try {
      final response = await http.post(
        historyUrl,
        headers: {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print("Failed to send history record: ${response.body}");
      }
    } catch (e) {
      print("Error sending history record: $e");
    }
  }
  ...
  String _capitalizeStatus(String? status) {
    ...
    if (status.toLowerCase() == 'full') {
      return 'FULL';
    } else if (status.toLowerCase() == 'emptied') {
      return 'AVAILABLE';
    }
  ...
  Widget build(BuildContext context) {
    ...
    final confirmation_time =
        historyData!['confirmation_time'] ?? 'Unknown date';
                                  ...
                                  Expanded(
                                    child: ElevatedButton(
                                      ...
                                      style: ElevatedButton.styleFrom(
                                        ...
                                        elevation:
                                            3, // Slightly reduced shadow effect
                                        padding: EdgeInsets.symmetric(
                                            horizontal: 16.0,
                                            vertical: 8.0), // Reduced padding
                                        minimumSize: Size(100,
                                            36), // Smaller overall button size
                                      ),
                                      child: Text(
                                        'Empty now',
                                        style: TextStyle(
                                          fontSize:
                                              14.0, // Slightly smaller font size
                                          fontWeight: FontWeight.bold,
                                        ),
                                      ),
                                    ),
  ...
  String _formatDateTime(String dateTimeStr) {
    ...
    final hongKongTime =
        dateTime.add(Duration(hours: 8)); // Convert to Hong Kong time (UTC+8)
    return DateFormat('yyyy-MM-dd hh:mm:ss a').format(hongKongTime);
  }
